...
const NAME_KEY = 'assignment-radar.user-name';
...
const pressureBreakdown = (assignment: Assignment, allAssignments: Assignment[]): PressureBreakdown => {
  if (assignment.status === 'submitted') return { score: 0, time: 0, difficulty: 0, effort: 0, importance: 0, overlap: 0, nearbyCount: 0 };
  const days = dateDiff(assignment.dueDate);
  const time = days < 0 ? 40 : days === 0 ? 38 : days === 1 ? 32 : days <= 3 ? 25 : days <= 7 ? 16 : Math.max(4, 16 - Math.min(12, days - 7));
  const effort = Math.min(25, Math.round((assignment.effortMinutes / 240) * 25));
  const difficulty = { low: 4, medium: 7, high: 10 }[assignment.difficulty || 'medium'];
  const importance = { low: 5, medium: 10, high: 15 }[assignment.importance];
  const nearbyCount = allAssignments.filter((other) => {
    if (other.id === assignment.id || other.status === 'submitted') return false;
    return Math.abs(dateDiff(other.dueDate) - days) <= 1;
  }).length;
  const overlap = Math.min(15, nearbyCount * 5);
  return { score: Math.min(100, time + difficulty + effort + importance + overlap), time, difficulty, effort, importance, overlap, nearbyCount };
};

const pressureScore = (assignment: Assignment, allAssignments: Assignment[]) => pressureBreakdown(assignment, allAssignments).score;

const pressureTone = (score: number) => (score >= 75 ? 'high' : score >= 50 ? 'medium' : 'low');
...
function AppContent() {
  ...
  const [userName, setUserName] = useState(() => localStorage.getItem(NAME_KEY)?.trim() || '');
  const [nameEditorOpen, setNameEditorOpen] = useState(() => !localStorage.getItem(NAME_KEY)?.trim());
  ...
  const saveName = (nextName: string) => {
    ...
    notify(`Good to have you here, ${cleanName}.`);
    return true;
  };
  ...
  return (
    <QuerylessBoundary resetKey={location}>
      <AppShell userName={userName} theme={theme} onThemeToggle={() => setTheme(theme === 'light' ? 'dark' : 'light')} onChangeName={() => setNameEditorOpen(true)}>
        <Switch>
          <Route path="/">
            <DashboardPage assignments={assignments} userName={userName} onMarkSubmitted={markSubmitted} />
          </Route>
          ...
          <Route path="/settings">
            <SettingsPage userName={userName} onChangeName={() => setNameEditorOpen(true)} theme={theme} onThemeToggle={() => setTheme(theme === 'light' ? 'dark' : 'light')} onClear={clearAll} />
          </Route>
          <Route component={NotFound} />
        </Switch>
      </AppShell>
      {nameEditorOpen && <NameModal initialName={userName} required={!userName} onClose={userName ? () => setNameEditorOpen(false) : undefined} onSave={saveName} />}
...
function AppShell({ children, userName, theme, onThemeToggle, onChangeName }: { children: ReactNode; userName: string; theme: 'light' | 'dark'; onThemeToggle: () => void; onChangeName: () => void }) {
  return (
    <div className="app-shell flex">
      ...
      <aside className="sidebar desktop-sidebar fixed inset-y-0 left-0 z-30 flex w-[244px] flex-col px-5 py-6">
        ...
        <button onClick={onChangeName} className="focus-ring mt-4 flex items-center gap-2 rounded-xl px-3 py-2.5 text-left text-sm transition hover:bg-[hsl(var(--sidebar-accent))]" data-testid="button-change-name">
          <UserRound size={16} className="text-[hsl(var(--accent))]" />
          <span className="min-w-0 flex-1 truncate text-[hsl(var(--sidebar-foreground)/.75)]"><span className="block text-[10px] uppercase tracking-wider text-[hsl(var(--sidebar-foreground)/.45)]">Signed in as</span><span className="block truncate font-semibold text-[hsl(var(--sidebar-foreground))]">{userName || 'Student'}</span></span>
          <Pencil size={13} className="shrink-0 text-[hsl(var(--sidebar-foreground)/.45)]" />
        </button>
...
function DashboardPage({ assignments, userName, onMarkSubmitted }: { assignments: Assignment[]; userName: string; onMarkSubmitted: (id: string) => void }) {
  ...
  const focus = [...pending].sort((a, b) => pressureScore(b, assignments) - pressureScore(a, assignments)).slice(0, 3);
  ...
  return <div className="space-y-9">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
     ...
     <section data-testid="section-todays-focus"><div className="flex items-end justify-between"><div><div className="mono-font text-[10px] font-bold uppercase tracking-[.2em] text-[hsl(var(--primary))]">Your next moves</div><h2 className="display-font mt-2 text-3xl font-bold tracking-[-.03em]">Today’s focus</h2></div><Link href="/assignments" className="focus-ring text-sm font-bold text-[hsl(var(--primary))]" data-testid="link-see-all-assignments">See all <ArrowUpRight size={14} className="ml-1 inline" /></Link></div>{focus.length ? <div className="mt-5 grid gap-3 lg:grid-cols-3">{focus.map((item) => <AssignmentCard key={item.id} assignment={item} allAssignments={assignments} compact onMarkSubmitted={onMarkSubmitted} />)}</div> : <EmptyState compact />}</section>
  </div>;
}

function AssignmentCard({ assignment, allAssignments = [], compact = false, onOpen, onMarkSubmitted }: { assignment: Assignment; allAssignments?: Assignment[]; compact?: boolean; onOpen?: () => void; onMarkSubmitted?: (id: string) => void }) {
  const urgency = urgencyLabel(assignment);
  const pressure = pressureBreakdown(assignment, allAssignments);
  const tone = pressureTone(pressure.score);
  return <article className={`card-lift stagger-in group relative rounded-2xl border border-[hsl(var(--card-border))] bg-[hsl(var(--card))] ${compact ? 'p-5' : 'p-5 sm:p-6'}`} data-testid={`card-assignment-${assignment.id}`} onClick={onOpen}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2 text-xs font-bold text-[hsl(var(--primary))]"><span className="h-1.5 w-1.5 rounded-full bg-current" />{assignment.subject}</div><h3 className="mt-2 truncate text-[17px] font-bold tracking-[-.02em]">{assignment.title}</h3></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${urgency.tone === 'overdue' ? 'bg-[hsl(var(--destructive)/.12)] text-[hsl(var(--destructive))]' : urgency.tone === 'today' ? 'bg-[hsl(var(--accent))] text-[hsl(var(--accent-foreground))]' : urgency.tone === 'done' ? 'bg-[hsl(151_37%_43%/.12)] text-[hsl(151_37%_35%)]' : 'bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))]'}`}>{urgency.label}</span></div>{!compact && assignment.description && <p className="mt-3 line-clamp-2 text-sm leading-5 text-[hsl(var(--muted-foreground))]">{assignment.description}</p>}<div className={`mt-4 flex items-center justify-between rounded-xl px-3 py-2.5 ${tone === 'high' ? 'bg-[hsl(var(--destructive)/.1)]' : tone === 'medium' ? 'bg-[hsl(var(--accent)/.14)]' : 'bg-[hsl(var(--primary)/.08)]'}`} data-testid={`pressure-summary-${assignment.id}`}><span className="flex items-center gap-2 text-xs font-bold"><Gauge size={14} className={tone === 'high' ? 'text-[hsl(var(--destructive))]' : tone === 'medium' ? 'text-[hsl(var(--accent-foreground))]' : 'text-[hsl(var(--primary))]'} /> Pressure score</span><span className={`mono-font text-sm font-bold ${tone === 'high' ? 'text-[hsl(var(--destructive))]' : tone === 'medium' ? 'text-[hsl(var(--accent-foreground))]' : 'text-[hsl(var(--primary))]'}`}>{pressure.score}/100</span></div><div className="mt-4 flex items-center justify-between gap-3 border-t border-[hsl(var(--border)/.7)] pt-4"><div className="flex items-center gap-3 text-xs text-[hsl(var(--muted-foreground))]"><span className="flex items-center gap-1"><Clock3 size={13} /> {formatMinutes(assignment.effortMinutes)}</span><span className={`flex items-center gap-1 font-semibold ${assignment.priority === 'high' ? 'text-[hsl(var(--destructive))]' : ''}`}><span className="h-1.5 w-1.5 rounded-full bg-current" /> {assignment.priority} priority</span></div>{assignment.status !== 'submitted' && onMarkSubmitted && <button className="focus-ring rounded-lg p-1.5 text-[hsl(var(--muted-foreground))] opacity-70 transition hover:bg-[hsl(151_37%_43%/.12)] hover:text-[hsl(151_37%_35%)] hover:opacity-100" title="Mark as submitted" onClick={(event) => { event.stopPropagation(); onMarkSubmitted(assignment.id); }} data-testid={`button-submit-${assignment.id}`}><Check size={16} /></button>}</div></article>;
}
...
function AssignmentsPage({ assignments, onAdd, onUpdate, onDelete, onMarkSubmitted }: { assignments: Assignment[]; onAdd: (data: Omit<Assignment, 'id' | 'createdAt'>) => void; onUpdate: (id: string, data: Omit<Assignment, 'id' | 'createdAt'>) => void; onDelete: (id: string) => void; onMarkSubmitted: (id: string) => void }) {
  ...
  const filtered = useMemo(() => assignments.filter((item) => {
    ...
    }).sort((a, b) => pressureScore(b, assignments) - pressureScore(a, assignments)), [assignments, filter, search]);
  ...
  return <div className="space-y-8">
    ...
    <div className="flex flex-col gap-3 lg:flex-row"><label className="relative flex-1"><Search size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[hsl(var(--muted-foreground))]" /><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search title, subject, or teacher" className="focus-ring h-11 w-full rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--card))] pl-10 pr-4 text-sm outline-none transition focus:border-[hsl(var(--primary))]" data-testid="input-search-assignments" /></label><div className="flex items-center gap-2 overflow-x-auto pb-1"><Filter size={15} className="ml-1 shrink-0 text-[hsl(var(--muted-foreground))]" />{(['all', 'today', 'soon', 'high', 'not-started', 'in-progress', 'submitted'] as FilterKey[]).map((key) => <button key={key} onClick={() => setFilter(key)} className={`focus-ring flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-2 text-xs font-bold transition ${filter === key ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--primary)/.4)]'}`} data-testid={`button-filter-${key}`}><span>{key === 'not-started' ? 'Not started' : key === 'in-progress' ? 'In progress' : key === 'high' ? 'High priority' : key === 'all' ? 'All' : key === 'today' ? 'Due today' : key === 'soon' ? 'Due soon' : 'Submitted'}</span><span className="mono-font text-[10px] opacity-70">{counts[key]}</span></button>)}</div></div>
     {filtered.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((item) => <AssignmentCard key={item.id} assignment={item} allAssignments={assignments} onOpen={() => setDetail(item)} onMarkSubmitted={onMarkSubmitted} />)}</div> : <EmptyState />}
    {editor && <AssignmentForm initial={editor === 'new' ? undefined : editor} onClose={() => setEditor(null)} onSave={(data) => { if (editor === 'new') onAdd(data); else onUpdate(editor.id, data); setEditor(null); }} />}
     {detail && <AssignmentDetail assignment={detail} allAssignments={assignments} onClose={() => setDetail(null)} onEdit={() => { setEditor(detail); setDetail(null); }} onDelete={() => { if (window.confirm('Remove this assignment from your radar?')) { onDelete(detail.id); setDetail(null); } }} onMarkSubmitted={() => { onMarkSubmitted(detail.id); setDetail({ ...detail, status: 'submitted' }); }} />}
  </div>;
}
...
function AssignmentForm({ initial, onClose, onSave }: { initial?: Assignment; onClose: () => void; onSave: (data: Omit<Assignment, 'id' | 'createdAt'>) => void }) {
  ...
  const [difficulty, setDifficulty] = useState<Priority>(initial?.difficulty || 'medium');
...
function Modal({ title, onClose, children }: { title: string; onClose?: () => void; children: ReactNode }) {
  return <div className="fixed inset-0 z-40 flex items-end justify-center bg-[hsl(var(--foreground)/.45)] p-0 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true"><div className="page-enter max-h-[94dvh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-[hsl(var(--card))] p-5 shadow-[var(--shadow-xl)] sm:rounded-3xl sm:p-8"><div className="mb-7 flex items-center justify-between"><h2 className="display-font text-2xl font-bold">{title}</h2>{onClose && <button onClick={onClose} className="focus-ring rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" data-testid="button-close-modal"><X size={19} /></button>}</div>{children}</div></div>;
}

function NameModal({ initialName, required, onClose, onSave }: { initialName: string; required: boolean; onClose?: () => void; onSave: (name: string) => boolean }) {
  const [mode, setMode] = useState<'login' | 'signup'>(initialName ? 'login' : 'signup');
  ...
  return <Modal title={isEditing ? 'Change your name' : mode === 'login' ? 'Welcome back' : 'Create your radar'} onClose={required ? undefined : onClose}><form onSubmit={submit} className="space-y-6" data-testid="form-name-access"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[hsl(var(--accent)/.22)] text-[hsl(var(--primary))]"><UserRound size={28} /></div>{!isEditing && <div className="grid grid-cols-2 rounded-xl bg-[hsl(var(--muted)/.7)] p-1"><button type="button" onClick={() => setMode('login')} className={`focus-ring flex items-center justify-center gap-2 rounded-lg px-3 py-2.5 text-sm font-bold ${mode === 'login' ? 'bg-[hsl(var(--card))] shadow-sm' : 'text-[hsl(var(--muted-foreground))]'}`} data-testid="button-login-mode"><LogIn size={15} /> Log in</button><button type="button" onClick={() => setMode('signup')} className={`focus-ring flex items-center justify-center gap-2 rounded-lg px-3 py-2.5 text-sm font-bold ${mode === 'signup' ? 'bg-[hsl(var(--card))] shadow-sm' : 'text-[hsl(var(--muted-foreground))]'}`} data-testid="button-signup-mode"><UserPlus size={15} /> Sign up</button></div>}<div className="text-center"><p className="mx-auto max-w-sm text-sm leading-6 text-[hsl(var(--muted-foreground))]">{isEditing ? 'This is the name we use to greet you around Assignment Radar.' : mode === 'login' ? 'Enter your name to pick up where you left off.' : 'Start with your name. No password, account, or extra setup needed.'}</p></div><Field label="Your name"><input autoFocus value={name} onChange={(event) => { setName(event.target.value); setError(''); }} placeholder="e.g. Alex Morgan" className="form-input" data-testid="input-user-name" /></Field>{error && <p className="flex items-center gap-2 text-sm font-semibold text-[hsl(var(--destructive))]" data-testid="status-name-error"><TriangleAlert size={15} />{error}</p>}<div className="flex justify-end gap-2 border-t border-[hsl(var(--border))] pt-5"><button type="submit" className="focus-ring inline-flex items-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-5 py-2.5 text-sm font-bold text-[hsl(var(--primary-foreground))]" data-testid="button-save-name">{isEditing ? 'Save name' : mode === 'login' ? 'Log in' : 'Create my radar'} <ChevronRight size={15} /></button></div><p className="text-center text-[11px] text-[hsl(var(--muted-foreground))]">Your assignments stay in this browser.</p></form></Modal>;
}

function AssignmentDetail({ assignment, allAssignments, onClose, onEdit, onDelete, onMarkSubmitted }: { assignment: Assignment; allAssignments: Assignment[]; onClose: () => void; onEdit: () => void; onDelete: () => void; onMarkSubmitted: () => void }) {
  const urgency = urgencyLabel(assignment);
  const pressure = pressureBreakdown(assignment, allAssignments);
  const tone = pressureTone(pressure.score);
  return <Modal title="Assignment details" onClose={onClose}><div className="space-y-6" data-testid={`detail-assignment-${assignment.id}`}><div><div className="flex items-center gap-2 text-sm font-bold text-[hsl(var(--primary))]"><BookOpen size={15} />{assignment.subject}</div><h3 className="display-font mt-2 text-3xl font-bold tracking-[-.04em]">{assignment.title}</h3><p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">{assignment.teacher || 'No teacher added'}</p></div><div className={`rounded-2xl p-5 ${tone === 'high' ? 'bg-[hsl(var(--destructive)/.1)]' : tone === 'medium' ? 'bg-[hsl(var(--accent)/.16)]' : 'bg-[hsl(var(--primary)/.08)]'}`} data-testid={`pressure-detail-${assignment.id}`}><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-sm font-bold"><Gauge size={17} /> Assignment pressure</div><p className="mt-1 max-w-sm text-xs leading-5 text-[hsl(var(--muted-foreground))]">A transparent score that weighs time, difficulty, effort, importance, and nearby deadlines.</p></div><div className={`mono-font text-3xl font-bold ${tone === 'high' ? 'text-[hsl(var(--destructive))]' : tone === 'medium' ? 'text-[hsl(var(--accent-foreground))]' : 'text-[hsl(var(--primary))]'}`}>{pressure.score}<span className="text-sm opacity-50">/100</span></div></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-[hsl(var(--foreground)/.1)]"><div className={`h-full rounded-full ${tone === 'high' ? 'bg-[hsl(var(--destructive))]' : tone === 'medium' ? 'bg-[hsl(var(--accent))]' : 'bg-[hsl(var(--primary))]'}`} style={{ width: `${pressure.score}%` }} /></div><div className="mt-4 grid grid-cols-2 gap-3 text-xs sm:grid-cols-5"><PressureFactor label="Time left" value={pressure.time} /><PressureFactor label="Difficulty" value={pressure.difficulty} /><PressureFactor label="Effort" value={pressure.effort} /><PressureFactor label="Importance" value={pressure.importance} /><PressureFactor label="Nearby work" value={pressure.overlap} /></div>{pressure.nearbyCount > 0 && <p className="mt-4 text-xs font-semibold text-[hsl(var(--muted-foreground))]">{pressure.nearbyCount} other assignment{pressure.nearbyCount === 1 ? '' : 's'} due within a day adds pressure here.</p>}</div><div className="grid grid-cols-2 gap-3 sm:grid-cols-5"><DetailItem label="Due" value={urgency.label} /><DetailItem label="Effort" value={formatMinutes(assignment.effortMinutes)} /><DetailItem label="Difficulty" value={assignment.difficulty || 'medium'} /><DetailItem label="Priority" value={assignment.priority} /><DetailItem label="Importance" value={assignment.importance} /></div>{assignment.description && <div><h4 className="text-xs font-bold uppercase tracking-wide text-[hsl(var(--muted-foreground))]">Description</h4><p className="mt-2 text-sm leading-6">{assignment.description}</p></div>}{assignment.notes && <div className="rounded-xl bg-[hsl(var(--accent)/.18)] p-4"><h4 className="text-xs font-bold uppercase tracking-wide">Notes</h4><p className="mt-2 text-sm leading-6">{assignment.notes}</p></div>}{assignment.attachmentName && <div className="flex items-center gap-3 rounded-xl border border-[hsl(var(--border))] p-3 text-sm"><FileText size={17} className="text-[hsl(var(--primary))]" /><span className="truncate">{assignment.attachmentName}</span></div>}{assignment.submissionLink && <button onClick={() => window.open(assignment.submissionLink, '_blank', 'noopener,noreferrer')} className="focus-ring flex w-full items-center justify-between rounded-xl border border-[hsl(var(--border))] p-3 text-left text-sm font-bold transition hover:border-[hsl(var(--primary)/.4)] hover:bg-[hsl(var(--muted)/.5)]" data-testid="button-open-submission-link"><span className="flex items-center gap-2"><Link2 size={17} className="text-[hsl(var(--primary))]" /> Open submission link</span><ExternalLink size={15} /></button>}<div className="flex flex-wrap justify-between gap-2 border-t border-[hsl(var(--border))] pt-5"><button onClick={onDelete} className="focus-ring inline-flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-bold text-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive)/.1)]" data-testid="button-delete-assignment"><Trash2 size={16} /> Delete</button><div className="flex gap-2"><button onClick={onEdit} className="focus-ring inline-flex items-center gap-2 rounded-xl border border-[hsl(var(--border))] px-3 py-2.5 text-sm font-bold hover:bg-[hsl(var(--muted))]" data-testid="button-edit-assignment"><Pencil size={15} /> Edit</button>{assignment.status !== 'submitted' && <button onClick={onMarkSubmitted} className="focus-ring inline-flex items-center gap-2 rounded-xl bg-[hsl(var(--primary))] px-3 py-2.5 text-sm font-bold text-[hsl(var(--primary-foreground))]" data-testid="button-detail-submit"><Check size={15} /> Mark submitted</button>}</div></div></div></Modal>;
}
...
function PressureFactor({ label, value }: { label: string; value: number }) {
  return <div data-testid={`pressure-factor-${label.toLowerCase().replaceAll(' ', '-')}`}><div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-wide text-[hsl(var(--muted-foreground))]"><span>{label}</span><span>{value}</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[hsl(var(--foreground)/.1)]"><div className="h-full rounded-full bg-current" style={{ width: `${Math.min(100, (value / 40) * 100)}%` }} /></div></div>;
}
...
function SettingsPage({ userName, onChangeName, theme, onThemeToggle, onClear }: { userName: string; onChangeName: () => void; theme: 'light' | 'dark'; onThemeToggle: () => void; onClear: () => void }) {
  return <div className="space-y-8"><PageIntro eyebrow="Your space, your rules" title="Settings" description="Assignment Radar is local-first. Your coursework lives in this browser, under your control." /><div className="max-w-3xl space-y-4"><section className="rounded-2xl border border-[hsl(var(--card-border))] bg-[hsl(var(--card))] p-6" data-testid="section-profile"><div className="flex items-start gap-4"><div className="rounded-xl bg-[hsl(var(--primary)/.1)] p-3 text-[hsl(var(--primary))]"><UserRound size={19} /></div><div className="flex-1"><h2 className="font-bold">Your profile</h2><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">A name is all Assignment Radar needs to feel like your space.</p><div className="mt-5 flex items-center justify-between gap-4 rounded-xl bg-[hsl(var(--muted)/.65)] px-4 py-3"><div className="min-w-0"><div className="text-[10px] font-bold uppercase tracking-wide text-[hsl(var(--muted-foreground))]">Display name</div><div className="mt-1 truncate text-sm font-bold">{userName || 'Student'}</div></div><button onClick={onChangeName} className="focus-ring inline-flex shrink-0 items-center gap-2 rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-3 py-2 text-xs font-bold hover:bg-[hsl(var(--muted))]" data-testid="button-settings-change-name"><Pencil size={13} /> Change name</button></div></div></div></section><section className="rounded-2xl border border-[hsl(var(--card-border))] bg-[hsl(var(--card))] p-6" data-testid="section-appearance"><div className="flex items-start gap-4"><div className="rounded-xl bg-[hsl(var(--accent)/.22)] p-3"><Sun size={19} /></div><div className="flex-1"><h2 className="font-bold">Appearance</h2><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">Choose the light of your desk.</p><div className="mt-5 flex gap-2"><button onClick={() => theme === 'dark' && onThemeToggle()} className={`focus-ring inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-bold ${theme === 'light' ? 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.08)] text-[hsl(var(--primary))]' : 'border-[hsl(var(--border))] text-[hsl(var(--muted-foreground))]'}`} data-testid="button-theme-light"><Sun size={15} /> Light</button><button onClick={() => theme === 'light' && onThemeToggle()} className={`focus-ring inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-bold ${theme === 'dark' ? 'border-[hsl(var(--accent))] bg-[hsl(var(--accent)/.12)] text-[hsl(var(--accent))]' : 'border-[hsl(var(--border))] text-[hsl(var(--muted-foreground))]'}`} data-testid="button-theme-dark"><Moon size={15} /> Dark</button></div></div></div></section><section className="rounded-2xl border border-[hsl(var(--destructive)/.25)] bg-[hsl(var(--card))] p-6" data-testid="section-data-controls"><div className="flex items-start gap-4"><div className="rounded-xl bg-[hsl(var(--destructive)/.1)] p-3 text-[hsl(var(--destructive))]"><Trash2 size={19} /></div><div className="flex-1"><h2 className="font-bold">Clear local data</h2><p className="mt-1 max-w-lg text-sm leading-6 text-[hsl(var(--muted-foreground))]">Remove every assignment from this browser. This cannot be undone, so make sure you really mean it.</p><button onClick={() => { if (window.confirm('Clear every assignment from Assignment Radar? This cannot be undone.')) onClear(); }} className="focus-ring mt-5 inline-flex items-center gap-2 rounded-xl border border-[hsl(var(--destructive)/.4)] px-4 py-2.5 text-sm font-bold text-[hsl(var(--destructive))] transition hover:bg-[hsl(var(--destructive)/.1)]" data-testid="button-clear-all"><Trash2 size={15} /> Clear all data</button></div></div></section><div className="flex items-center gap-2 px-1 text-xs text-[hsl(var(--muted-foreground))]" data-testid="text-local-storage-note"><ShieldCheck size={14} /> Stored only on this device · no account required</div></div></div>;
}
